  ...
  if (req.method === "GET") {
    try {
      ...
      const moneyTransactionsQuery = `
        ...
        WHERE user_id = $1
      `;
      const moneyTransactionsResult = await db.query(moneyTransactionsQuery, [
        userId,
      ]);
      ...
      const stockTransactionsQuery = `
        ...
        WHERE user_id = $1
      `;
      const stockTransactionsResult = await db.query(stockTransactionsQuery, [
        userId,
      ]);
      ...
      const stocks = await Stock.find({
        _id: { $in: stockIds.map((id) => id.replace(/^"|"$/g, "")) },
      });
      ...
      const formattedStockTransactions = stockTransactions.map(
        (transaction) => {
          const stockDetails =
            stockMap[transaction.stock_id.replace(/^"|"$/g, "")];
          return {
            id: transaction.id,
      ...
      const formattedMoneyTransactions = moneyTransactions.map(
        (transaction) => ({
          id: transaction.id,
          amount: transaction.amount,
          order_executed: transaction.order_executed,
