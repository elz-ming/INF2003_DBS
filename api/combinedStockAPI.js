...
const { insertNewsData, insertSentimentData, insertPriceData } = require('../insert'); // Importing the required functions
    ...
    try {
        ...
        for (const ticker of Object.values(ticker_codes)) {
            ...
            const newsData = await new Promise((resolve, reject) => {
                https.request(newsOptions, (apiRes) => {
                    const chunks = [];
            ...
            const sentimentData = await new Promise((resolve, reject) => {
                https.request(sentimentOptions, (apiRes) => {
                    const chunks = [];
                    apiRes.on('data', (chunk) => {
                        chunks.push(chunk);
                    });
                ...
                const apiReq = https.request(priceOptions, (apiRes) => {
                    ...
                    apiRes.on("end", async () => {
                        ...
                        try {
                            ...
                            resolve({ tickerCode, lastRecord: { date: dateLast, open: openLast, high: highLast, low: lowLast, close: closeLast, volume: volumeLast } });
                        } catch (err) {
